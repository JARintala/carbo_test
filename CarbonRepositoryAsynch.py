...
class CarbonRepositoryAsynch:
    ...
    async def populate_container_items(self, container_obj, carbonData: CarbonData):
        # <create_item>
        try:
            data = json.dumps(carbonData, default=vars)
            data_ad_dict = json.loads(data)
            await container_obj.create_item(data_ad_dict)
    ...
    # <method_read_items>
    async def read_items(self, container_obj, carbonData: CarbonData):
        # <read_item>
        try:
    ...
    # <save_carbonData>
    async def save_carbonData(self, carbonData: CarbonData):
        # <create_cosmos_client>
        async with cosmos_client(self.endpoint, credential=self.key) as client:
            # </create_cosmos_client>
            try:
                # create a database
                ...
                await self.populate_container_items(container_obj, carbonData)
                # read the just populated item using their id and partition key
                await self.read_items(container_obj, carbonData)
                # Query these item using the SQL query syntax.
                query = "SELECT * FROM c WHERE c.id = " + carbonData.id
                await self.query_items(container_obj, query)
            except Exception as e:
